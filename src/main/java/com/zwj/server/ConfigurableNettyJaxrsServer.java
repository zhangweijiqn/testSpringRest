...
public class ConfigurableNettyJaxrsServer extends NettyJaxrsServer {
    ...
    public void start() {
        ...
        if (sslContext == null) {
            factory = new HttpServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, true, Lists.<ChannelHandler>newArrayList());
        } else {
            factory = new HttpsServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, true, Lists.<ChannelHandler>newArrayList(), sslContext);
        }

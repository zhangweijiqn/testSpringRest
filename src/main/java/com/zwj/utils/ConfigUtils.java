...
//	}
    ...
    public static Properties getProperties() {
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    String path = System.getProperty(Constants.SERVER_PROPERTIES_KEY);
                    if (path == null || path.length() == 0) {
                        path = System.getenv(Constants.SERVER_PROPERTIES_KEY);
                        if (path == null || path.length() == 0) {
                            path = Constants.DEFAULT_SERVER_PROPERTIES;
                        }
                    }
     ...
     *
     * @param fileName       properties file name. for example: <code>server.properties</code>, <code>METE-INF/conf/foo.properties</code>
    ...
    public static Properties loadProperties(String fileName, boolean allowMultiFile, boolean optional) {
        ...
        if (!allowMultiFile) {
            if (list.size() > 1) {
                String errMsg = String.format("only 1 %s file is expected, but %d server.properties files found on class path: %s",
